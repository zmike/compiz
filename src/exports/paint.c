...
{
#if 0
   GLfloat h = screen->height;

   GLfloat p1[2] = { region->extents.x1, h - region->extents.y2 };
   GLfloat p2[2] = { region->extents.x2, h - region->extents.y1 };

   GLfloat halfW = output->width / 2.0;
   GLfloat halfH = output->height / 2.0;

   GLfloat cx = output->region.extents.x1 + halfW;
   GLfloat cy = (h - output->region.extents.y2) + halfH;

   GLfloat top[4] = { 0.0, halfH / (cy - p1[1]), 0.0, 0.5 };
   GLfloat bottom[4] = { 0.0, halfH / (cy - p2[1]), 0.0, 0.5 };
...
drawWindowGeometry(CompWindow *w)
{
   ...
   compiz_glapi->glDrawArrays(0x0007, 0, w->vCount);
