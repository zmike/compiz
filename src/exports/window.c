...
updateWindowClassHints(CompWindow *w)
{
   ...
   if (ec->icccm.class)
     {
        w->resClass = strdup(ec->icccm.class);
     }
}
...
updateTransientHint(CompWindow *w)
{
   ...
   transientFor = compiz_win_to_client(w)->icccm.transient_for;
...
updateIconGeometry(CompWindow *w)
{
   ...
   if (!e_pixmap_is_x(compiz_win_to_client(w)->pixmap)) return;
           ...
           Window id)
{
   ...
   ec = e_pixmap_find_client(E_PIXMAP_TYPE_X, id);
   return ec ? ec->icccm.state : 0;
}
               ...
               Window id)
{
   ...
   ec = e_pixmap_find_client(E_PIXMAP_TYPE_X, id);
   if (!ec)
     ec = e_pixmap_find_client(E_PIXMAP_TYPE_WL, id);
   if (!ec) return 0;

   return client_state_to_compiz_state(ec);
}
              ...
              Window id)
{
   ...
   if (!ec)
     ec = e_pixmap_find_client(E_PIXMAP_TYPE_WL, id);
   return win_type_to_compiz(ec->netwm.type);
}
   ...
   if (!w->pixmap)
     {
        ...
        cp = compiz_win_to_client(w)->pixmap;
        w->pixmap = e_pixmap_pixmap_get(cp);
        e_pixmap_size_get(cp, &w->width, &w->height);
...
EINTERN void
handleAddClient(E_Client *ec)
{
   E_Zone *zone;
   CompDisplay *d;
   CompScreen *s;

   zone = ec->zone ? : e_zone_current_get();
   for (d = core.displays; d; d = d->next)
     {
        for (s = d->screens; s; s = s->next)
          if (s->screenNum == zone->num)
            {
               E_Client *bec;

               bec = e_client_below_get(ec);
               addWindow(s, window_get(ec), bec ? window_get(bec) : 0);
               break;
            }
     }
}
          ...
          Window aboveId)
{
   ...
   w = (CompWindow *)calloc(1, sizeof (CompWindow));
   if (!w)
     return;
   ...
   if (e_pixmap_is_x(ec->pixmap))
   ...
   else
     {
        ...
        w->attrib.x = ec->client.x;
        w->attrib.y = ec->client.y;
        w->attrib.width = ec->client.w;
        w->attrib.height = ec->client.h;
     }
        ...
        if (!w->attrib.override_redirect)
          {
             ...
             if (compiz_win_to_client(w)->iconic)
               {
             ...
             else
               {
                  if (w->wmType & (CompWindowTypeDockMask |
                    ...
                    {
                       if (w->desktop != 0xffffffff)
                         w->desktop = ec->desk->x * ec->desk->y + ec->desk->y;
